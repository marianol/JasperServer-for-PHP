...
for ($reportnumber=0; $reportnumber < 125 ; $reportnumber++ ) { 
	$screen ='';
...
try {
    ...
    $outputReports[$reportnumber] = $reportUUID;
    } catch (Exception $e) 
        {
...
foreach ($outputReports as $Report => $UUID) {
    ...
    try {    
        ...
        $requestAttach = 'report/' . $UUID . '?file=attachment';
        //$attachfile = $WSRestRaw->get($requestAttach);    	
        $status = ($Report == $reportfile) ? 'PASS' : 'FAIL';
    	$screen .=  "$Report -- $reportfile --- $status - $requestReport <br />";
